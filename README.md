...
/sdd:spec-req auth    # "인증은 JWT, 7일 만료"  →  의도가 스펙에 기록됨
...
| 단계 | 커맨드 | 산출물 | 핵심 질문 |
|------|--------|--------|-----------|
| ① 방향 설정 | `/sdd:steering` | product / tech / structure.md | 무엇을 왜 만드는가 |
| ② 요구사항 | `/sdd:spec-req` | requirements.md (REQ-NNN) | 무엇을 만들어야 하는가 |
| ③ 설계 | `/sdd:spec-design` | design.md | 어떻게 만들 것인가 |
| ④ 계획 검토 | 사용자 OK | — | 이대로 만들어도 되는가 |
| ⑤ 태스크 | `/sdd:spec-tasks` | tasks.md (TDD 순서) | 무엇부터 만들 것인가 |
| ⑥ 구현 | `/sdd:spec-impl` | 코드 + 테스트 | 실제로 만든다 |
| ⑦ 검증 | `/sdd:spec-val` + `/sdd:spec-delta` | 갭 리포트 | 제대로 만들었는가 |
| ⑧ 회고/개선 | `/sdd:retro` + `/sdd:spec-capture` | lessons/*.md | 다음엔 어떻게 더 잘할 것인가 |

> **핵심 원칙**: 갭 발견 비용은 단계가 앞일수록 저렴하다.  
> 설계 단계에서 버그를 잡으면 구현 후 수정보다 훨씬 빠르다.
...
```
/sdd:steering → /sdd:spec-req → /sdd:spec-design → /sdd:spec-tasks → /sdd:spec-impl
```
...
`@impl` 태그는 **개발 도구가 아니라 유지보수 도구**예요.  
처음 만들 때가 아니라, "나중에 바꿀 때"를 위해 붙이는 거예요.

**권장 패턴:**

| 단계 | 작업 |
|------|------|
| 처음 개발 | `/sdd:spec-impl` 또는 직접 코딩 → 완성 우선 |
| 개발 완료 후 | `@impl` 태그 추가 → 추적 준비 (`spec-tag` 또는 수동) |
| 요구사항 변경 시 | `/sdd:spec-update` → `spec-delta` → 코드 수정 |
...
| 커맨드 | 설명 |
|--------|------|
| `/sdd:spec-req <feature>` | 요구사항 생성 |
| `/sdd:spec-design <feature>` | 설계 문서 생성 |
| `/sdd:spec-plan <feature>` | req + design + tasks 한 번에 |
| `/sdd:spec-tasks <feature>` | 태스크 목록 생성 |
| `/sdd:spec-impl <feature> [task]` | TDD 구현 |
| `/sdd:spec-auto <설명>` | 완전 자동 구현 (overnight용) |
| `/sdd:spec-decomp "<Epic 설명>"` | Epic → Feature→Story 계층 분해, 각 Feature 스펙 폴더 자동 생성 |
| `/sdd:spec-update <feature> "<요청>"` | 자연어 변경 요청 → 신규/수정 자동 분류 → spec-delta/spec-impl 연결 |
| `/sdd:retro [feature]` | 애자일 회고 (Keep/Problem/Try) + 품질 지표 + 교훈 자동 저장 |
| `/sdd:spec-index <feature>` | requirements.md에 INDEX 섹션 생성 (100줄+ 대용량 스펙용) |
...
| `/sdd:spec-val <feature>` | 스펙-코드 정합성 검증 |
...
| `/sdd:steer-trim [file]` | 긴 스티어링 분리 (≤100줄 유지) |
